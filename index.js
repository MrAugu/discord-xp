...
let db;
let DBName;
const AvailableDBs = ["quick.db", "mongoose"];
...
class DiscordXp {
  ...
  static async setDB(dbName, dbInstance) {
    if (!dbName) throw new TypeError("No DB type was provided.");
    if (!AvailableDBs.includes(dbName))
      throw new TypeError("Invalid DB type provided.");
    DBName = dbName;
    if (
      dbName.toLowerCase() === "mongoose" ||
      dbName.toLowerCase() === "mongodb"
    ) {
